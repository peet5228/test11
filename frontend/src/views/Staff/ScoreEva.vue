    ...
    <v-container>
        <v-row >
            <v-col cols="12">
                ...
                <v-alert type="info" v-else-if="user.status_eva === 1">ยังไม่ได้กรอกแบบประเมิน</v-alert>
...
const fetchUser = async () =>{
    try{
        const res = await axios.get(`${Eva}/scoreEva/${id_eva}`,{headers:{Authorization:`Bearer ${token}`}})
        user.value = res.data
    }catch(err){
...
const fetchTopics = async () =>{
    try{
        const res = await axios.get(`${Eva}/scoreEva/indicate/${id_eva}`,{headers:{Authorization:`Bearer ${token}`}})
