...
router.get('/:id_eva',verifyToken,requireRole('กรรมการประเมิน'),async (req,res) => {
    try{
        ...
        const [rows] = await db.query(`select * from tb_eva e,tb_member m,tb_system s,tb_commit c where c.id_member=? and c.id_eva=? and c.id_eva=e.id_eva and e.id_member=m.id_member and e.id_sys=s.id_sys`,
            [id_member,id_eva]
        )
        res.json(rows[0])
    }catch(err){
